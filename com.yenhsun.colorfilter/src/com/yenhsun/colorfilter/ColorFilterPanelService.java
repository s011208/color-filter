...
public class ColorFilterPanelService extends Service {
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(Intent.ACTION_SCREEN_ON)) {
				boolean enable = getSharedPreferences(
						MainActivity.SHARF_FILE_NAME, 0).getBoolean(
						MainActivity.SHARF_KEY_ENABLE_FILTER, false);
				int filterColor = getSharedPreferences(
						MainActivity.SHARF_FILE_NAME, 0).getInt(
						MainActivity.SHARF_KEY_FILTER_COLOR,
						MainActivity.DEFAULT_FILTER_COLOR);
				if (enable) {
					try {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (mWm != null && mColorFilterPanel != null && mWmParams != null) {
			boolean enable = getSharedPreferences(MainActivity.SHARF_FILE_NAME,
					0).getBoolean(MainActivity.SHARF_KEY_ENABLE_FILTER, false);
			int filterColor = getSharedPreferences(
					MainActivity.SHARF_FILE_NAME, 0).getInt(
					MainActivity.SHARF_KEY_FILTER_COLOR,
					MainActivity.DEFAULT_FILTER_COLOR);
			if (intent.getExtras() != null) {
				...
				if (callFromNotification) {
					...
					getSharedPreferences(MainActivity.SHARF_FILE_NAME, 0)
							.edit()
							.putBoolean(MainActivity.SHARF_KEY_ENABLE_FILTER,
									enable).commit();
			...
			Toast.makeText(
					this,
					enable ? R.string.toast_show_filter
							: R.string.toast_hide_filter, Toast.LENGTH_SHORT)
					.show();
		}
	...
	public WindowManager.LayoutParams getWindowManagerParamsSettings() {
		...
		int deviceHeight = mScreenHeight + statusBarHeight
